...
export const BUILTIN_SKILLS: Map<string, SkillDefinition> = new Map([
  ...
  ['web-researcher', {
    ...
    icon: 'search',
    securityLevel: 'safe',
    allowedTools: ['browser_open', 'browser_search', 'open_url', 'create_task', 'get_weather'],
  ...
  ['task-manager', {
    ...
    icon: 'tasks',
    securityLevel: 'safe',
    allowedTools: ['create_task', 'get_weather', 'browser_search', 'browser_open', 'open_url'],
    deniedTools: ['terminal_run', 'send_email', 'telegram_send', 'telegram_read', 'discord_send', 'discord_read', 'whatsapp_send', 'slack_send', 'slack_read'],
    safeBinsOnly: true,
    safetyRules: [
  ...
  ['safe-coder', {
    ...
    allowedTools: ['terminal_run', 'browser_open', 'browser_search', 'open_url', 'create_task', 'get_weather'],
